...
class Package:  # pylint:disable=too-many-instance-attributes
    ...
    def _load_fields(self, variables: bash.Variables) -> None:
        ...
        installdepends_raw = _pop_field_indexed(variables, "installdepends", [])
        self.variables["installdepends"] = installdepends_raw
        self.installdepends = []

        for dep_raw in installdepends_raw:
            dep = Dependency.parse(dep_raw or "")

            if dep.kind != DependencyKind.Host:
                raise RecipeError(
                    "Only host packages are supported in the 'installdepends' \
field"
                )

            self.installdepends.append(dep)
...
custom functions with '_'"
    ...
    def control_fields(self) -> str:
        ...
        if self.installdepends:
            control += (
                "Depends: "
                + ", ".join(
                    dep.to_debian() for dep in self.installdepends if dep
                )
                + "\n"
            )
